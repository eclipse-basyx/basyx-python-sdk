...
class ProvidersTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.aas1 = model.AssetAdministrationShell(
            model.AssetInformation(global_asset_id="http://example.org/TestAsset1/"),
            "urn:x-test:aas1",
        )
        self.aas2 = model.AssetAdministrationShell(
            model.AssetInformation(global_asset_id="http://example.org/TestAsset2/"),
            "urn:x-test:aas2",
        )
    ...
    def test_store_retrieve(self) -> None:
        for store_class in self._STORE_CLASSES:
            with self.subTest(store=store_class.__name__):
                store: model.AbstractObjectStore[
                    model.Identifier, model.Identifiable
                ] = store_class([self.aas1])
                store.add(self.aas2)
                ...
                with self.assertRaises(KeyError) as cm:
                    store.add(aas3)
                self.assertEqual(
                    "'Identifiable object with same id urn:x-test:aas1 is already "
    ...
    def test_store_update(self) -> None:
        for store_class in self._STORE_CLASSES:
            with self.subTest(store=store_class.__name__):
                store1: model.AbstractObjectStore[
                    model.Identifier, model.Identifiable
                ] = store_class()
                store1.add(self.aas1)
                store2: model.AbstractObjectStore[
                    model.Identifier, model.Identifiable
                ] = store_class()
    ...
    def test_store_sync(self) -> None:
        for store_class in self._STORE_CLASSES:
            with self.subTest(store=store_class.__name__):
                ...
                self.assertEqual(
                    store.sync([self.aas1, self.aas2], overwrite=False), (2, 0, 0)
                )
                self.assertIn(self.aas1, store)
                ...
                self.assertEqual(
                    store.sync([self.aas1, self.submodel1], overwrite=True), (1, 1, 0)
                )

                self.assertEqual(
                    store.sync([self.aas1, self.submodel2], overwrite=False), (1, 0, 1)
                )
    ...
    def test_provider_multiplexer(self) -> None:
        ...
        submodel_identifiable_store: model.DictIdentifiableStore[model.Identifiable] = (
            model.DictIdentifiableStore()
        )
        submodel_identifiable_store.add(self.submodel1)
        submodel_identifiable_store.add(self.submodel2)

        multiplexer: model.ObjectProviderMultiplexer[
            model.Identifier, model.Identifiable
        ] = model.ObjectProviderMultiplexer(
            [aas_identifiable_store, submodel_identifiable_store]
        )
        ...
        self.assertEqual(
            "'Key could not be found in any of the 2 consulted registries.'",
            str(cm.exception),
        )
