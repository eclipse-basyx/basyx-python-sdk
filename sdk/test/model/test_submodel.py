...
class SubmodelElementListTest(unittest.TestCase):
    def test_constraints(self):
        ...
        model.SubmodelElementList(
            "test_list", model.AnnotatedRelationshipElement, {are}
        )
        ...
        with self.assertRaises(model.AASConstraintViolation) as cm:
            model.SubmodelElementList(
                "test_list", model.MultiLanguageProperty, [mlp1, mlp2]
            )
        self.assertEqual(
...
class BasicEventElementTest(unittest.TestCase):
    def test_constraints(self):
        ...
        self.assertEqual(
            "max_interval is not applicable if direction = input!", str(cm.exception)
        )
