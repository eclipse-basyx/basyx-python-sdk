...
class CouchDBBackendTest(unittest.TestCase):
    def setUp(self) -> None:
        self.coch_identifiable_store = couchdb.CouchDBIdentifiableStore(TEST_CONFIG['couchdb']['url'],
                                                                        TEST_CONFIG['couchdb']['database'])
        couchdb.register_credentials(TEST_CONFIG["couchdb"]["url"],
                                     TEST_CONFIG["couchdb"]["user"],
                                     TEST_CONFIG["couchdb"]["password"])
        self.coch_identifiable_store.check_database()

    def tearDown(self) -> None:
        self.coch_identifiable_store.clear()

    def test_object_store_add(self):
        test_object = create_example_submodel()
        self.coch_identifiable_store.add(test_object)
    ...
    def test_retrieval(self):
        ...
        test_object_retrieved = self.coch_identifiable_store.get_item('https://acplt.org/Test_Submodel')
        self.assertIs(test_object, test_object_retrieved)

        # When retrieving it again, we should still get the same object
        del test_object
        test_object_retrieved_again = self.coch_identifiable_store.get_item('https://acplt.org/Test_Submodel')
    ...
    def test_example_submodel_storing(self) -> None:
        ...
        self.coch_identifiable_store.add(example_submodel)
        self.assertEqual(1, len(self.coch_identifiable_store))
        self.assertIn(example_submodel, self.coch_identifiable_store)

        # Restore example submodel and check data
        submodel_restored = self.coch_identifiable_store.get_item('https://acplt.org/Test_Submodel')
        ...
        self.coch_identifiable_store.discard(submodel_restored)
        self.assertNotIn(example_submodel, self.coch_identifiable_store)
    ...
    def test_iterating(self) -> None:
        ...
        for item in example_data:
            self.coch_identifiable_store.add(item)

        self.assertEqual(5, len(self.coch_identifiable_store))
        ...
        for item in self.coch_identifiable_store:
    ...
    def test_key_errors(self) -> None:
        ...
        retrieved_submodel = self.coch_identifiable_store.get_item('https://acplt.org/Test_Submodel')
        self.coch_identifiable_store.discard(example_submodel)
        with self.assertRaises(KeyError) as cm:
            self.coch_identifiable_store.get_item('https://acplt.org/Test_Submodel')
