...
class AASDataChecker(DataChecker):
    ...
    def _check_extension_equal(
        self, object_: model.Extension, expected_object: model.Extension
    ):
        """
    ...
    def _check_referable_equal(
        self, object_: model.Referable, expected_object: model.Referable
    ):
        """
        ...
        self.check_attribute_equal(
            object_, "display_name", expected_object.display_name
        )
    ...
    def _check_identifiable_equal(
        ...
        if (
            object_.administration is not None
            and expected_object.administration is not None
        ):
    ...
    def _check_has_kind_equal(
        self, object_: model.HasKind, expected_object: model.HasKind
    ):
        """
    ...
    def _check_qualifiable_equal(
        ...
        for expected_element in expected_object.qualifier:
            ...
            if self.check(
                element is not None, "{} must exist".format(repr(expected_element))
            ):
    ...
    def _check_has_data_specification_equal(
        self,
        object_: model.HasDataSpecification,
    ...
    def _check_submodel_elements_equal_unordered(
        ...
        for expected_element in expected_value.value:
            try:
                ...
                self.check(
                    False,
                    "Submodel Element {} must exist".format(repr(expected_element)),
                )
    ...
    def check_property_equal(
        self, object_: model.Property, expected_value: model.Property
    ):
        """
    ...
    def check_multi_language_property_equal(
        self,
        object_: model.MultiLanguageProperty,
    ...
    def check_resource_equal(
        self, object_: model.Resource, expected_value: model.Resource
    ):
        """
    ...
    def check_submodel_element_list_equal(
        self,
        object_: model.SubmodelElementList,
        ...
        self._check_abstract_attributes_submodel_element_equal(object_, expected_value)
        self.check_attribute_equal(
            object_, "order_relevant", expected_value.order_relevant
        )
        self.check_attribute_equal(
    ...
    def check_relationship_element_equal(
        self,
        object_: model.RelationshipElement,
    ...
    def check_annotated_relationship_element_equal(
        ...
        self.check(
            found_elements == set(),
            "Annotated Reference {} must not have extra references".format(
                repr(object_)
            ),
            value=found_elements,
        )

    def _check_reference_equal(
        self, object_: model.Reference, expected_value: model.Reference
    ):
        """
    ...
    def _find_extra_elements_by_attribute(
        self,
        object_list: Union[Set, List],
        search_list: Union[Set, List],
        *attribute: str,
    ) -> Set:
        """
        ...
        for object_list_element in object_list:
            ...
            for search_list_element in search_list:
                for attr in attribute:
                    if getattr(object_list_element, attr) != getattr(
                        search_list_element, attr
                    ):
                        found = False
                    else:
                        found = True
    ...
    def _find_extra_namespace_set_elements_by_attribute(
        self,
        object_list: model.NamespaceSet,
        search_list: model.NamespaceSet,
        attr_name: str,
    ) -> Set:
        """
        ...
        found_elements = set()
        for object_list_element in object_list:
            element = search_list.get(
                attr_name, getattr(object_list_element, attr_name)
            )
            if element is None:
                found_elements.add(object_list_element)
        return found_elements
    ...
    def check_operation_equal(
        self, object_: model.Operation, expected_value: model.Operation
    ):
        """
        ...
        for input_nss, expected_nss, attr_name in (
            ...
            (
                object_.output_variable,
                expected_value.output_variable,
                "output_variable",
            ),
            (
                object_.in_output_variable,
                expected_value.in_output_variable,
                "in_output_variable",
            ),
        ):
            self.check_contained_element_length(
    ...
    def check_capability_equal(
        self, object_: model.Capability, expected_value: model.Capability
    ):
        """
    ...
    def check_entity_equal(self, object_: model.Entity, expected_value: model.Entity):
        ...
        self.check_attribute_equal(
            object_, "global_asset_id", expected_value.global_asset_id
        )
        ...
        for expected_element in expected_value.statement:
            element = object_.get_referable(expected_element.id_short)
            self.check(
                element is not None, f"Entity {repr(expected_element)} must exist"
            )
    ...
    def _check_specific_asset_ids_equal(
        ...
        for expected_pair in expected_value:
            ...
            if self.check(
                pair is not None, f"SpecificAssetId {repr(expected_pair)} must exist"
            ):
                self.check_specific_asset_id(pair, expected_pair)  # type: ignore

        found_elements = self._find_extra_object(
            object_, expected_value, model.SpecificAssetId
        )
        self.check(
            found_elements == set(),
    ...
    def check_basic_event_element_equal(
        ...
        self.check_attribute_equal(
            object_, "message_topic", expected_value.message_topic
        )
        self.check_attribute_equal(
            object_, "message_broker", expected_value.message_broker
        )
    ...
    def check_submodel_equal(
        self, object_: model.Submodel, expected_value: model.Submodel
    ):
        """
    ...
    def _check_qualifier_equal(
        self, object_: model.Qualifier, expected_value: model.Qualifier
    ):
        """
    ...
    def check_asset_information_equal(
        ...
        if object_.default_thumbnail and expected_value.default_thumbnail:
            self.check_resource_equal(
                object_.default_thumbnail, expected_value.default_thumbnail
            )
        else:
            if object_.default_thumbnail:
                self.check(
                    expected_value.default_thumbnail is not None,
                    "defaultThumbnail object {} must exist".format(
                        repr(object_.default_thumbnail)
                    ),
    ...
    def check_asset_administration_shell_equal(
        ...
        for expected_ref in expected_value.submodel:
            ...
            if self.check(
                ref is not None,
                "Submodel Reference {} must exist".format(repr(expected_ref)),
            ):
    ...
    def check_concept_description_equal(
        self,
        object_: model.ConceptDescription,
        ...
        for expected_ref in expected_value.is_case_of:
            ...
            if self.check(
                ref is not None,
                "Concept Description Reference {} must exist".format(
                    repr(expected_ref)
                ),
            ):
                self._check_reference_equal(ref, expected_ref)  # type: ignore
    ...
    def _check_data_specification_iec61360_equal(
        ...
        """
        self.check_attribute_equal(
            object_, "preferred_name", expected_value.preferred_name
        )
    ...
    def _check_value_list_equal(
        ...
        """
        for expected_pair in expected_value:
            pair = self._find_element_by_attribute(
                expected_pair, object_, "value", "value_id"
            )
            self.check(
                pair is not None,
                "ValueReferencePair[value={}, value_id={}] must exist".format(
                    expected_pair.value, expected_pair.value_id
                ),
            )
    ...
    def check_attribute_equal(
        ...
        if (
            attribute_name
            in ("value_type", "value_type_list_element", "type_value_list_element")
    ...
    def check_element_in(
        self, object_: model.Referable, parent: object, **kwargs
    ) -> bool:
        """
    ...
    def check_attribute_is_none(
        self, object_: object, attribute_name: str, **kwargs
    ) -> bool:
        """
