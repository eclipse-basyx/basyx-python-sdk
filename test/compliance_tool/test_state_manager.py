...
class ComplianceToolStateManagerTest(unittest.TestCase):
    ...
    def test_logs(self):
        ...
        self.assertEqual(manager.get_step_string(1), 'SUCCESS: test 2')
        self.assertEqual(manager.get_step_string(1, 1), 'SUCCESS: test 2')
        self.assertNotEqual(-1, manager.get_step_string(1, 2).find('INFO:  OK  : Assertion test ()'))

        checker.check(2 == 1, 'Assertion test 2')
        manager.add_log_records_from_data_checker(checker)
        self.assertEqual(1, len(manager.get_error_logs_from_step(1)))
        self.assertEqual(manager.get_step_string(1), 'FAILED: test 2')
        self.assertNotEqual(-1, manager.get_step_string(1, 1).find('ERROR: FAIL: Assertion test 2 ()'))
        self.assertNotEqual(-1, manager.get_step_string(1, 2).find('INFO:  OK  : Assertion test ()'))
