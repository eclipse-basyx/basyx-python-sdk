...
class ServerAASFromJsonDecoder(AASFromJsonDecoder):
    ...
    def _construct_asset_administration_shell_descriptor(
            ...
            object_class=model.AssetAdministrationShellDescriptor) -> model.AssetAdministrationShellDescriptor:
    ...
    def _construct_submodel_descriptor(
            cls, dct: Dict[str, object],
            object_class=model.SubmodelDescriptor) -> model.SubmodelDescriptor:
        ret = object_class(id_=_get_ts(dct, 'id', str),
                           endpoints=[])
        cls._amend_abstract_attributes(ret, dct)
        for endpoint_dct in _get_ts(dct, 'endpoints', list):
...
class ServerAASToJsonEncoder(AASToJsonEncoder):
    ...
    def _get_aas_class_serializers(cls) -> Dict[Type, Callable]:
        ...
        serializers.update({
            model.AssetAdministrationShellDescriptor: cls._asset_administration_shell_descriptor_to_json,
            model.SubmodelDescriptor: cls._submodel_descriptor_to_json,
            model.Endpoint: cls._endpoint_to_json,
            model.ProtocolInformation: cls._protocol_information_to_json,
            model.AssetLink: cls._asset_link_to_json
        })
        return serializers
    ...
    @classmethod
    def _asset_administration_shell_descriptor_to_json(cls, obj: model.AssetAdministrationShellDescriptor) -> \
            Dict[str, object]:
        """
    ...
    @classmethod
    def _endpoint_to_json(cls, obj: model.Endpoint) -> Dict[str, object]:
        data = cls._abstract_classes_to_json(obj)
